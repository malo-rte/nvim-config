-- eslint LSP config without nvim-lspconfig dependency
--
-- Requires: npm i -g vscode-langservers-extracted

local lsp = vim.lsp

local eslint_config_files = {
	".eslintrc",
	".eslintrc.js",
	".eslintrc.cjs",
	".eslintrc.yaml",
	".eslintrc.yml",
	".eslintrc.json",
	"eslint.config.js",
	"eslint.config.mjs",
	"eslint.config.cjs",
	"eslint.config.ts",
	"eslint.config.mts",
	"eslint.config.cts",
}

local function insert_package_json(root_files, field, fname)
	local path_sep = package.config:sub(1, 1)

	-- Find nearest package.json upwards from current file
	local pkg = vim.fs.find("package.json", {
		path = fname,
		upward = true,
		type = "file",
		limit = 1,
	})[1]

	if not pkg then
		return root_files
	end

	local ok, fd = pcall(io.open, pkg, "r")
	if not ok or not fd then
		return root_files
	end

	for line in fd:lines() do
		if line:find(field, 1, true) then
			table.insert(root_files, "package.json")
			break
		end
	end
	fd:close()

	return root_files
end

---@type vim.lsp.Config
return {
	cmd = { "vscode-eslint-language-server", "--stdio" },
	filetypes = {
		"javascript",
		"javascriptreact",
		"javascript.jsx",
		"typescript",
		"typescriptreact",
		"typescript.tsx",
		"vue",
		"svelte",
		"astro",
		"htmlangular",
	},
	workspace_required = true,

	on_attach = function(client, bufnr)
		-- Keep the FixAll command; you can still call it manually
		vim.api.nvim_buf_create_user_command(bufnr, "LspEslintFixAll", function()
			client:request_sync("workspace/executeCommand", {
				command = "eslint.applyAllFixes",
				arguments = {
					{
						uri = vim.uri_from_bufnr(bufnr),
						version = lsp.util.buf_versions[bufnr],
					},
				},
			}, nil, bufnr)
		end, {})
	end,

	root_dir = function(bufnr, on_dir)
		-- project root markers
		local root_markers = {
			"package-lock.json",
			"yarn.lock",
			"pnpm-lock.yaml",
			"bun.lockb",
			"bun.lock",
		}

		if vim.fn.has("nvim-0.11.3") == 1 then
			root_markers = { root_markers, { ".git" } }
		else
			root_markers = vim.list_extend(root_markers, { ".git" })
		end

		-- exclude deno projects
		if vim.fs.root(bufnr, { "deno.json", "deno.jsonc", "deno.lock" }) then
			return
		end

		local project_root = vim.fs.root(bufnr, root_markers) or vim.fn.getcwd()

		-- detect whether this buffer actually uses eslint
		local filename = vim.api.nvim_buf_get_name(bufnr)

		local eslint_config_files_with_package_json =
			insert_package_json(vim.list_extend({}, eslint_config_files), "eslintConfig", filename)

		local is_buffer_using_eslint = vim.fs.find(eslint_config_files_with_package_json, {
			path = filename,
			type = "file",
			limit = 1,
			upward = true,
			stop = vim.fs.dirname(project_root),
		})[1]

		if not is_buffer_using_eslint then
			return
		end

		on_dir(project_root)
	end,

	-- See https://github.com/microsoft/vscode-eslint/blob/main/server/src/eslintServer.ts
	settings = {
		validate = "on",
		---@diagnostic disable-next-line: assign-type-mismatch
		packageManager = nil,
		useESLintClass = false,
		experimental = {
			useFlatConfig = false,
		},
		codeActionOnSave = {
			enable = false,
			mode = "all",
		},
		format = true,
		quiet = false,
		onIgnoredFiles = "off",
		rulesCustomizations = {},
		run = "onType",
		problems = {
			shortenToSingleLine = false,
		},
		nodePath = "",
		workingDirectory = { mode = "auto" },
		codeAction = {
			disableRuleComment = {
				enable = true,
				location = "separateLine",
			},
			showDocumentation = {
				enable = true,
			},
		},
	},

	before_init = function(_, config)
		local root_dir = config.root_dir
		if not root_dir then
			return
		end

		config.settings = config.settings or {}
		config.settings.workspaceFolder = {
			uri = root_dir,
			name = vim.fn.fnamemodify(root_dir, ":t"),
		}

		-- detect flat config files (anything eslint*.config.* under root, excluding node_modules)
		local flat_config_files = vim.tbl_filter(function(file)
			return file:match("config")
		end, eslint_config_files)

		for _, file in ipairs(flat_config_files) do
			local found_files = vim.fn.globpath(root_dir, file, true, true)

			local filtered = {}
			for _, found_file in ipairs(found_files) do
				if not found_file:match("[/\\]node_modules[/\\]") then
					table.insert(filtered, found_file)
				end
			end

			if #filtered > 0 then
				config.settings.experimental = config.settings.experimental or {}
				config.settings.experimental.useFlatConfig = true
				break
			end
		end

		-- Yarn 2 (PnP) support
		local pnp_cjs = root_dir .. "/.pnp.cjs"
		local pnp_js = root_dir .. "/.pnp.js"
		if type(config.cmd) == "table" and (vim.uv.fs_stat(pnp_cjs) or vim.uv.fs_stat(pnp_js)) then
			config.cmd = vim.list_extend({ "yarn", "exec" }, config.cmd --[[@as table]])
		end
	end,

	handlers = {
		["eslint/openDoc"] = function(_, result)
			if result then
				vim.ui.open(result.url)
			end
			return {}
		end,
		["eslint/confirmESLintExecution"] = function(_, result)
			if not result then
				return
			end
			return 4 -- approved
		end,
		["eslint/probeFailed"] = function()
			vim.notify("ESLint probe failed.", vim.log.levels.WARN)
			return {}
		end,
		["eslint/noLibrary"] = function()
			vim.notify("Unable to find ESLint library.", vim.log.levels.WARN)
			return {}
		end,
	},
}
